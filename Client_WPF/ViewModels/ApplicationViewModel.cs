...
namespace Client_WPF.ViewModels
{
    public class ApplicationViewModel : INotifyPropertyChanged
    {
        ...
        {
            db = new AreasdbContext();
            dbSize = db.Areas.Count();

            Areas = new ObservableCollection<AreaInfo>();

            Cache = new Caching<AreaInfo>();
            CacheList = new ObservableCollection<AreaInfo>();

            Favorites = JDeserializer<ObservableCollection<AreaInfo>>.Deser(ReadWriter<ObservableCollection<AreaInfo>>.Read())
                ?? new ObservableCollection<AreaInfo>();
        }

        private readonly AreasdbContext db;
        ...
        private Caching<AreaInfo> Cache { get; set; }
        public ObservableCollection<AreaInfo> CacheList { get; set; }

        public ObservableCollection<AreaInfo> Favorites { get; set; }
            ...
            get
            {
                return downCommand ??
                  (downCommand = new RelayCommand(obj =>
                  {
                      ...
                      if (sizeList + 20 >= dbSize)
                      {
                          for (int i = sizeList; i < dbSize; i++)
                          {
                              Areas.Add(db.Areas.Include(s => s.WorkingHours).FirstOrDefault(p => p.Id == i));
                          }
                      }
                      else
                      {
                          for (int i = sizeList; i < sizeList + 20; i++)
                          {
                              Areas.Add(db.Areas.Include(s => s.WorkingHours).FirstOrDefault(p => p.Id == i));
                          }
                      }
                  },
                 (obj) => Areas.Count < dbSize));
            }
        }
        ...
        public RelayCommand AddCommand
        {
            get
            {
                return addCommand ??
                  ...
                  {
                      AreaInfo elem = obj as AreaInfo;

                      if (elem != null && elem.Id != 0)
                      {
                          ReadWriter<List<AreaInfo>>.Write(new List<AreaInfo>() { elem });
                      }
                      else if (elem != null)
                      {
        ...
        public RelayCommand UpdateCommand
        {
            get
            {
                return updateCommand ??
                  ...
                  {
                      List<AreaInfo> temp_list = JDeserializer<List<AreaInfo>>.Deser(ReadWriter<List<AreaInfo>>.Read());
                      ...
                      if (temp_list != null)
                      {
                          ...
                          {
                              bool found;
                              foreach (AreaInfo itemT in temp_list)
                              {
                                  found = false;

                                  foreach (var itemD in Favorites)
                                  {
                                       if (itemD.Location == itemT.Location)
                                        found = true;
                                  }
